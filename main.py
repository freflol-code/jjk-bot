...
STATUS_PAYLOAD_PREFIX = "status_buy_"
...
def format_active_buffs(user_id):
...
def _get_current_step_info(user_id):
    chapter = story.get_current_chapter(user_id)
    if not chapter:
        return None
...
def puzzle_keyboard(puzzle_id):
    ...
    if ptype == "sequence":
        ...
        for i, sym in enumerate(symbols):
            rows.append([InlineKeyboardButton(sym, callback_data=f"story_puzzle:{puzzle_id}:tap:{i}")])
    elif ptype == "reaction":
        rows.append([InlineKeyboardButton("▶️ Начать", callback_data=f"story_puzzle:{puzzle_id}:start")])
        rows.append([InlineKeyboardButton("🛑 Тормоз", callback_data=f"story_puzzle:{puzzle_id}:brake")])
...
def main_keyboard(user_id):
    ...
    rows.append([InlineKeyboardButton("🚇 Метро", callback_data="subway_open")])
...
def quests_period_keyboard(period):
    return InlineKeyboardMarkup([
...
def gacha_list_keyboard(user_id):
...
def gacha_rarity_keyboard(user_id, short):
    rarity = RARITY_FROM_SHORT.get(short)
    if not rarity:
        return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="gacha_list")]])
    learned = database.get_player_techniques(user_id)
    techs = []
    for row in learned:
...
def gacha_clan_menu_keyboard(user_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🎰 Крутка клана ({CLAN_GACHA_ROLL_COST}💠)", callback_data="gacha_clan_roll1")],
...
def gacha_ce_menu_keyboard(user_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🎰 1 крутка ({CE_GACHA_ROLL_COST}💠)", callback_data="gacha_ce_roll1"),
         InlineKeyboardButton(f"🎰 10 круток ({CE_GACHA_ROLL_COST_X10}💠)", callback_data="gacha_ce_roll10")],
        [InlineKeyboardButton("📖 Мои типы ПЭ", callback_data="gacha_my_ce")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="gacha_hub")],
    ])


def my_clans_keyboard(user_id):
    ...
    for row in owned:
        ...
        if not c:
            continue
        if key == active:
            rows.append([InlineKeyboardButton(f"✅ {c['emoji']} {c['name']} (снять)", callback_data="clan_clear")])
        else:
            rows.append([InlineKeyboardButton(f"{c['emoji']} {c['name']} — надеть", callback_data=f"clan_set:{key}")])
    if not owned:
...
def my_ce_keyboard(user_id):
    ...
    for row in owned:
        ...
        if not t:
            continue
        if key == active:
            rows.append([InlineKeyboardButton(f"✅ {t['emoji']} {t['name']} (снять)", callback_data="ce_clear")])
        else:
            rows.append([InlineKeyboardButton(f"{t['emoji']} {t['name']} ({t['rarity']}) — надеть", callback_data=f"ce_set:{key}")])
    if not owned:
        rows.append([InlineKeyboardButton("— пусто —", callback_data="noop")])
...
def my_heavenly_keyboard(user_id):
    ...
    for row in owned:
        ...
        if not h:
            continue
        if key == active:
            rows.append([InlineKeyboardButton(f"✅ {h['emoji']} {h['name']} (снять)", callback_data="heavenly_clear")])
        else:
            rows.append([InlineKeyboardButton(f"{h['emoji']} {h['name']} — надеть", callback_data=f"heavenly_set:{key}")])
    if not owned:
        rows.append([InlineKeyboardButton("— пусто —", callback_data="noop")])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data="gacha_clan_menu")])
    return InlineKeyboardMarkup(rows)
...
def inventory_keyboard(user_id):
...
def shop_sell_keyboard(npc_id, user_id):
...
def gear_buy_list_keyboard(user_id):
    rows = []
...
def gear_owned_keyboard(user_id):
    ...
    for idx, name in enumerate(owned):
        ...
        if name == equipped_name:
            rows.append([InlineKeyboardButton(f"✅ {w['emoji']} {name} (снять)", callback_data="gear_unequip")])
        else:
            rows.append([InlineKeyboardButton(f"{w['emoji']} {name} — экипировать", callback_data=f"gear_equip:{idx}")])
    if not owned:
        rows.append([InlineKeyboardButton("— пусто —", callback_data="noop")])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data="gear_menu:hakari")])
    return InlineKeyboardMarkup(rows)
...
def story_menu_keyboard(user_id):
    rows = []
    chapter = story.get_current_chapter(user_id)
    player = database.get_or_create_player(user_id, "")
    if chapter:
        ...
        if player["level"] < min_level:
            rows.append([InlineKeyboardButton(f"🔒 Нужен уровень {min_level}", callback_data="noop")])
        else:
...
def story_temp_keyboard(user_id):
    rows = []
    info = _get_current_step_info(user_id)
    if info:
        _, step = info
        if step["type"] == "puzzle":
            rows.append([InlineKeyboardButton("🧩 Решить головоломку", callback_data="story_temp_puzzle")])
        elif step["type"] == "kill_boss":
            rows.append([InlineKeyboardButton("👁 Войти к боссу", callback_data="story_temp_boss")])
    npc = story.get_current_npc(user_id)
    if npc:
        rows.append([InlineKeyboardButton(f"💬 Поговорить с {npc['name']}", callback_data="story_temp_talk")])
...
def profile_menu_keyboard(user_id):
    rows = [
    ...
    if database.has_vip(user_id):
        ...
        vip_label = "💎 VIP"
    rows.append([InlineKeyboardButton(vip_label, callback_data="vip_info"),
                 InlineKeyboardButton("💰 Донат", callback_data="donate_menu")])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_game")])
    return InlineKeyboardMarkup(rows)


def statuses_menu_keyboard(user_id):
    ...
    for key in statuses.get_visible_statuses(user_id):
        ...
        if key == active:
            ...
            marks.append("💰")
        label = f"{marks[0]} {st['emoji']} {st['styled']}"
        rows.append([InlineKeyboardButton(label, callback_data=f"status_open:{key}")])
    if active:
        rows.append([InlineKeyboardButton("❎ Снять активный статус", callback_data="status_clear")])
    if database.has_vip(user_id):
        hidden = database.is_vip_badge_hidden(user_id)
        if hidden:
            rows.append([InlineKeyboardButton("👁 Показать значок 💎", callback_data="vip_badge_toggle")])
        else:
            rows.append([InlineKeyboardButton("🙈 Скрыть значок 💎", callback_data="vip_badge_toggle")])
    rows.append([InlineKeyboardButton("⬅️ В профиль", callback_data="profile_menu")])
    return InlineKeyboardMarkup(rows)


def status_detail_keyboard(user_id, key):
    st = statuses.STATUSES.get(key)
    if not st:
        return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="statuses_menu")]])
    ...
    if active:
        ...
        rows.append([InlineKeyboardButton(
            f"💠 Купить за {st['gold_price']} очков",
            callback_data=f"status_buy_gold:{key}",
        )])
    rows.append([InlineKeyboardButton("⬅️ К списку статусов", callback_data="statuses_menu")])
    return InlineKeyboardMarkup(rows)


def subway_keyboard(user_id):
    player = database.get_or_create_player(user_id, "")
...
def raid_menu_keyboard(user_id):
    available = raid.get_available_fingers(user_id)
    rows = []
    for n in range(1, min(3, available) + 1):
        rows.append([InlineKeyboardButton(f"👺 Создать рейд на {n} 🩸", callback_data=f"raid_create:{n}")])
    rows.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_game")])
    return InlineKeyboardMarkup(rows)


def raid_lobby_keyboard(raid_id, user_id, bot_username):
...
def raid_battle_keyboard(raid_id, user_id):
    rows = []
    if raid.is_player_turn(raid_id, user_id):
        rows.append([InlineKeyboardButton("⚔️ Обычная атака", callback_data=f"raid_attack:{raid_id}")])
        eff = ce_types.get_effective_stats(user_id)
        if eff["max_ce"] > 0:
            equipped = gacha.get_equipped(user_id)
            for i, name in enumerate(equipped):
...
def raid_finished_keyboard():
    return InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ В меню", callback_data="back_to_game")]])


def truncate_caption(text, limit=CAPTION_LIMIT):
    if len(text) <= limit:
        return text
    suffix = "..."
...
async def render(query, context, text, reply_markup, image_path=None, force_new=False):
...
def district_image_for_x(x):
    return assets.get_biome_image(get_district_by_x(x)["id"])


async def send_effect_gif(context, chat_id, effect_key, ttl=EFFECT_GIF_TTL_SECONDS):
    if not effect_key:
        return
    path = None
...
def location_text(user_id, x):
    ...
    return (
        ...
        f"{weapon_line}{buffs_line}\n"
        f"{world_bounds_line(player['level'])}"
        f"{safe_line}{npc_line}{gojo_line}{story_hint}"
    )


def world_bounds_line(level):
...
def gacha_clan_menu_text(user_id):
    player = database.get_or_create_player(user_id, "")
    return (
...
def gacha_ce_menu_text(user_id):
    player = database.get_or_create_player(user_id, "")
    owned = database.get_ce_types(user_id)
    return (
...
def profile_text(user_id):
    ...
    if database.has_vip(user_id):
        ...
        until = database.get_vip_until(user_id)
        days_left = (until - int(time.time())) // 86400
        vip_line = f"\n💎 VIP: <b>{days_left} дн.</b>{badge_note}"
    else:
        vip_line = ""
    return (
        statuses.format_profile_header(user_id) + "\n\n"
...
async def _bot_username(context):
    try:
...
async def _render_raid_screen(query, context, raid_id, user_id):
    r = raid.get_raid(raid_id)
    if not r:
    ...
    if r["status"] == "lobby":
        ...
        if bot_name:
            text += f"\n\n🔗 Ссылка для друзей:\n<code>https://t.me/{bot_name}?start=raid_{raid_id}</code>"
...
async def _notify_raid_players(context, raid_id, user_ids, exclude=None):
    r = raid.get_raid(raid_id)
    if not r:
        return
    for uid in user_ids:
...
async def start(update, context):
    ...
    if args and args[0].startswith("raid_"):
        ...
        if rid:
            ...
            if not res.get("ok"):
                ...
                await update.message.reply_html(f"❌ {msg}", reply_markup=kb_for(user.id))
                return
            r = raid.get_raid(rid)
            if not r or r["status"] not in ("lobby", "battle"):
                await update.message.reply_html("❌ Рейд уже завершён или недоступен.", reply_markup=kb_for(user.id))
                return
            if r["status"] == "lobby":
    ...
    if _gojo_is_sealed(user.id):
        intro = (
            ...
            f"📖 <i>Годжо запечатан. Токийская школа магии держится на тебе. "
            f"Патрулируй районы, изгоняй проклятия, качайся у Хакари. "
            f"Ассоциация на тебя рассчитывает.</i>\n\n"
        )
    else:
        intro = (
            f"👋 Привет, {user.first_name}!\n\n"
...
async def send_inventory(user_id, sender):
    items = database.get_inventory(user_id)
    if not items:
    ...
    await sender("🎒 <b>Инвентарь</b>\n\n" + "\n".join(pretty), reply_markup=inventory_keyboard(user_id))
...
def _vip_description_text(user_id):
    until = database.get_vip_until(user_id)
    is_active = until > 0
    ...
    if is_active:
        ...
        hidden = database.is_vip_badge_hidden(user_id)
        if hidden:
            lines.append("🙈 Значок 💎 скрыт (можно включить в профиле)")
        lines.append("")
    ...
    lines.append("")
    lines.append("<i>Оплата через Telegram Stars. Продление возможно в любой момент — сроки суммируются.</i>")
    if is_active:
        lines.append("")
...
async def successful_payment_handler(update, context):
    ...
    if payload.startswith(DONATE_PAYLOAD_PREFIX):
        ...
        return
    if payload.startswith(STATUS_PAYLOAD_PREFIX):
        key = payload[len(STATUS_PAYLOAD_PREFIX):]
        res = statuses.grant_from_stars(user_id, key)
        if not res["ok"]:
            await msg.reply_html("❌ " + res["msg"], reply_markup=kb_for(user_id))
            return
        await msg.reply_html(
...
async def vip_info_handler(update, context):
    ...
    await render(query, context, _vip_description_text(user_id), InlineKeyboardMarkup(kb_rows))


def _donate_description_text(user_id):
    player = database.get_or_create_player(user_id, "")
    lines = [
...
async def status_buy_stars_handler(update, context, key):
    ...
    if database.has_player_status(user_id, key):
        await query.message.reply_text(
            ...
            reply_markup=status_detail_keyboard(user_id, key),
        )
        return
    try:
        await context.bot.send_invoice(
            chat_id=user_id,
            title=f"Статус: {st['styled']}",
            description=f"Косметический статус профиля.\n\n{st['desc']}",
            payload=f"{STATUS_PAYLOAD_PREFIX}{key}",
            provider_token="",
            currency="XTR",
            prices=[LabeledPrice(f"Статус {st['emoji']} {key}", st["star_price"])],
        )
    except Exception as e:
        logger.warning(f"Не удалось отправить инвойс статуса: {e}")
        await query.message.reply_text(f"❌ Ошибка оплаты: {e}")
...
async def button_handler(update, context):
    ...
    if (not in_raid_now and -25 <= x <= 25
    ...
    if data.startswith("status_open:"):
        key = data.split(":", 1)[1]
        text = statuses.format_detail(user_id, key)
        await render(query, context, text, status_detail_keyboard(user_id, key))
        return

    if data.startswith("status_buy_gold:"):
        key = data.split(":", 1)[1]
        res = statuses.buy_with_gold(user_id, key)
        prefix = "✅ " if res["ok"] else "❌ "
        text = prefix + res["msg"] + "\n\n" + statuses.format_list(user_id)
        await render(query, context, text, statuses_menu_keyboard(user_id))
        return

    if data.startswith("status_buy_stars:"):
        key = data.split(":", 1)[1]
        await status_buy_stars_handler(update, context, key)
        return

    if data.startswith("status_set:"):
        key = data.split(":", 1)[1]
        res = statuses.set_status(user_id, key)
        prefix = "✅ " if res["ok"] else "❌ "
        text = prefix + res["msg"] + "\n\n" + statuses.format_detail(user_id, key)
        await render(query, context, text, status_detail_keyboard(user_id, key))
        return

    if data == "status_clear":
        res = statuses.clear_status(user_id)
        prefix = "✅ " if res["ok"] else "❌ "
        text = prefix + res["msg"] + "\n\n" + statuses.format_list(user_id)
        await render(query, context, text, statuses_menu_keyboard(user_id))
        return

    if data == "vip_badge_toggle":
        res = statuses.toggle_vip_badge(user_id)
        prefix = "✅ " if res["ok"] else "❌ "
        text = prefix + res["msg"] + "\n\n" + statuses.format_list(user_id)
        await render(query, context, text, statuses_menu_keyboard(user_id))
        return
    ...
    if data == "boss_rush_start":
        res = boss_rush.start_rush(user_id)
        if not res["ok"]:
    ...
    if data == "boss_rush_continue":
        if not boss_rush.is_rush_active(user_id):
            text = "ℹ️ У тебя нет активного забега.\n\n" + boss_rush.format_menu(user_id)
            await render(query, context, text, boss_rush.rush_menu_keyboard(user_id))
            return
        encounter = database.get_encounter(user_id)
        if not encounter:
    ...
    if data == "raid_menu":
        if raid.get_active_raid_for_user(user_id):
            await _render_raid_screen(query, context, raid.get_active_raid_for_user(user_id)["id"], user_id)
            return
        available = raid.get_available_fingers(user_id)
        cd = raid.get_cooldown_until(user_id)
        if cd:
            left = cd - int(time.time())
            text = ("👺 <b>Рейд на Сукуну</b>\n\n"
        ...
        text = ("👺 <b>Рейд на Сукуну</b>\n\n"
                ...
                "⚠️ <i>У каждого участника должно быть не меньше пальцев, чем в рейде.</i>\n\n"
                + footer)
    ...
    if data == "raid_show":
        ...
        if not r:
            await render(query, context, "ℹ️ У тебя нет активного рейда.", kb_for(user_id))
            return
    ...
    if data.startswith("raid_start:"):
        ...
        await _notify_raid_players(context, rid, [p["user_id"] for p in parts], exclude=user_id)
        return
    ...
    if data.startswith("raid_attack:"):
        ...
        if not res.get("ok"):
            await render(query, context, "❌ " + res.get("msg", "Не удалось атаковать."), kb_for(user_id))
            return
        r = raid.get_raid(rid)
    ...
    if data == "domain_info":
        ...
        if technique_name:
            ...
            if cond["ok"]:
                ...
                lines.append("")
                lines.append("<i>Выполни условие в этом бою — и кнопка активации заработает.</i>")
    ...
    if data.startswith("subway_travel:"):
        ...
        await render(query, context, text, kb_for(user_id), image_path=district_image_for_x(new_x))
        return
    ...
    if data in ("move_left", "move_right"):
        ...
        await render(query, context, text, kb_for(user_id), image_path=district_image_for_x(raw_new_x))
        return
        ...
        if district.get("safe") and not database.get_encounter(user_id):
            if _gojo_is_sealed(user_id):
                quote = ("🛡 <b>Здесь безопасно.</b>\n\n"
                         "📖 <i>Барьер школы держится и без Годжо. Проклятия сюда не суются — проверено.</i>\n\n")
            else:
                quote = ("🛡 <b>Здесь безопасно.</b>\n\n"
        ...
        if idx < 0 or idx >= len(items):
            await render(query, context, "❌ Предмет недоступен. Открой инвентарь заново.", kb_for(user_id))
            return
        item = items[idx]
        if item["rarity"] != "расходник":
            await render(query, context, "❌ Этот предмет нельзя использовать.", inventory_keyboard(user_id))
            return
        ...
        is_seal = item["rarity"] == "призыв" or item["item_name"] in bosses.SUMMON_RECIPES
        if not is_seal:
            await render(query, context, "❌ Это не ритуальная печать.", inventory_keyboard(user_id))
            return
        item_name = item["item_name"]
        ...
        if not story.can_start_chapter(user_id):
            ...
            text = (f"🔒 <b>Нужен уровень {min_level}</b> (у тебя {player['level']}).\n\n" + tail)
            await render(query, context, text, story_menu_keyboard(user_id))
            return
        temp = story.get_temp_district(user_id)
        if not temp:
        ...
        if not puzzle:
            await render(query, context, "❌ Головоломка не найдена.", story_temp_keyboard(user_id))
            return
        if action == "start":
            if puzzle["type"] == "sequence":
                story.start_sequence_puzzle(user_id, pid)
                ...
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🛑 Тормоз", callback_data=f"story_puzzle:{pid}:brake")],
                ])
                await render(query, context, text, kb)
            return
        if action == "tap":
        ...
        await render(query, context, _donate_description_text(user_id), donate_menu_keyboard())
        return
        ...
        await render(query, context, gacha_clan_menu_text(user_id), gacha_clan_menu_keyboard(user_id))

    elif data == "gacha_ce_menu":
        await render(query, context, gacha_ce_menu_text(user_id), gacha_ce_menu_keyboard(user_id))

    elif data == "gacha_clan_roll1":
        cost = CLAN_GACHA_ROLL_COST
        player_now = database.get_or_create_player(user_id, "")
        if player_now["gold"] < cost:
            await render(query, context, f"❌ Не хватает очков. Нужно {cost}💠.", gacha_clan_menu_keyboard(user_id))
            return
        ...
        player_now = database.get_or_create_player(user_id, "")
        if player_now["gold"] < cost:
            await render(query, context, f"❌ Не хватает очков. Нужно {cost}💠.", gacha_ce_menu_keyboard(user_id))
            return
        database.add_gold(user_id, -cost)
        ...
        await render(query, context, ce_types.format_clan_list(user_id), my_clans_keyboard(user_id))

    elif data == "gacha_my_ce":
        await render(query, context, ce_types.format_ce_list(user_id), my_ce_keyboard(user_id))

    elif data == "gacha_my_heavenly":
        await render(query, context, ce_types.format_heavenly_list(user_id), my_heavenly_keyboard(user_id))
        ...
        await render(query, context, gacha_menu_text(user_id), gacha_menu_keyboard(), image_path=image_path)
        ...
        if context.user_data.get("gacha_from") == "shop":
            image_path = assets.get_npc_image("hakari_shop")
        await render(query, context, gacha_menu_text(user_id), gacha_list_keyboard(user_id), image_path=image_path)
        ...
        await render(query, context, "\n".join(lines), shop_buy_keyboard(npc_id), image_path=image_path)
        ...
        image_path = assets.get_npc_image(f"{npc_id}_shop")
        await render(query, context, "\n".join(lines), shop_sell_keyboard(npc_id, user_id), image_path=image_path)
