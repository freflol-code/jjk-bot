...
Имена игроков в лобби, в бою и в логах декорируются статусами профиля
и значком 💎 (см. statuses.decorate_name). Значок можно скрыть — тогда
он не покажется и здесь.
"""
import json
import random
import time
...
def _nick(user_id):
    """Декорированный ник с безопасной вставкой в HTML."""
    raw = database.get_or_create_player(user_id, "")["username"] or f"Игрок {user_id}"
    try:
        decorated = statuses.decorate_name(user_id, raw)
    except Exception:
        decorated = raw
    return decorated.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
...
def _set_raid_domain(raid_id, owner_id, domain_key, turns):
    conn = database.get_conn()
    conn.execute(
...
def _decrement_raid_domain(raid_id):
    conn = database.get_conn()
    cur = conn.cursor()
    cur.execute(
        "SELECT domain_key, domain_turns, domain_owner_id FROM raids WHERE id = ?",
    ...
    if new_turns == 0:
        conn.execute(
            "UPDATE raids SET domain_key = NULL, domain_turns = 0, domain_owner_id = NULL WHERE id = ?",
            (raid_id,),
        )
        conn.commit()
        return (None, None, 0)
    conn.execute("UPDATE raids SET domain_turns = ? WHERE id = ?", (new_turns, raid_id))
    conn.commit()
    return (row["domain_owner_id"], row["domain_key"], new_turns)


def _clear_raid_domain(raid_id):
    conn = database.get_conn()
    conn.execute(
        "UPDATE raids SET domain_key = NULL, domain_turns = 0, domain_owner_id = NULL WHERE id = ?",
        (raid_id,),
...
def _activate_raid_domain(raid, user_id, technique_name):
    ...
    if cur_key:
        ...
        owner_nick = _nick(owner_id) if owner_id else "?"
        raid["log"].append(
...
def _process_raid_domain_tick(raid):
...
def _get_finger_count(user_id):
    return database.count_item(user_id, "Палец Сукуны")


def get_cooldown_until(user_id):
    _ensure_tables()
    conn = database.get_conn()
    cur = conn.cursor()
...
def get_active_raid_for_user(user_id):
    _ensure_tables()
    conn = database.get_conn()
    cur = conn.cursor()
    cur.execute("""
...
def _save_raid(raid):
    ...
    conn.execute(
        ...
        (
            raid["status"], raid["boss_hp"], raid["turn_idx"],
            json.dumps(raid["log"], ensure_ascii=False),
            raid["finished_at"], raid["id"],
        ),
    )
    conn.commit()


def get_participants(raid_id):
    _ensure_tables()
    conn = database.get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM raid_participants WHERE raid_id = ? ORDER BY joined_at", (raid_id,))
    return [dict(r) for r in cur.fetchall()]


def create_raid(creator_id, fingers):
...
def leave_raid(raid_id, user_id):
    raid_row = _get_raid_row(raid_id)
    if not raid_row:
        return {"ok": False, "msg": "Рейд не найден."}
    raid = _row_to_raid(raid_row)
    ...
    conn = database.get_conn()
    conn.execute("DELETE FROM raid_participants WHERE raid_id = ? AND user_id = ?", (raid_id, user_id))
    conn.commit()
    parts = get_participants(raid_id)
    if not parts:
...
def start_battle(raid_id, user_id):
    raid_row = _get_raid_row(raid_id)
    if not raid_row:
        return {"ok": False, "msg": "Рейд не найден."}
    raid = _row_to_raid(raid_row)
...
def is_player_turn(raid_id, user_id):
...
def player_attack(raid_id, user_id, technique_name=None):
    raid = get_raid(raid_id)
    if not raid or raid["status"] != "battle":
...
def _send_all_to_school(raid_id):
...
def format_battle(raid_id, viewer_id):
    raid = get_raid(raid_id)
    if not raid:
        return "Рейд не найден."
    parts = get_participants(raid_id)
    ...
    for p in parts:
        ...
        lines.append(f"{marker} {nick}{owner_mark}: {p['hp']}/{p['max_hp']}")
    lines.append("")
    if cur_id:
        cur_nick = _nick(cur_id)
        if cur_id == viewer_id:
